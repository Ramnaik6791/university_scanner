...
def E_finland():
    url = "https://www.uef.fi/en/unit/school-of-computing"   # homepage url
    ...
    filename = "E_finland.txt"
    f = open(filename, "w")

    excel_filename = "E_finland.csv"
    ...
    d = soup.find_all('div', {'class':'link-block link-block--no-padding-left link-block--no-padding-right has-secondary-color-icon'})
    #print(d)
    # count = 0 
    ...
    for i in d:
        ...
        print(link)
        name = a.get('aria-label')                 # extracting prof name
        name = name.strip()
        print(name)
        try:
