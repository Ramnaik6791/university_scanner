...
def univIowa():
    url = "https://cs.uiowa.edu/people/faculty"   # homepage url
    ...
    filename = "univIowa.txt"
    f = open(filename, "w")

    excel_filename = "univIowa.csv"
    ...
    d = soup.find('div', {'class': 'block block-system block-system-main-block'})
    #print(d)
    dd=d.find_all('h3')
    print(dd)
    # count = 0 
    ...
    for i in dd:
        ...
        if a == None:
            continue
        link = "https://cs.uiowa.edu" + a.get('href')                # extracting prof page link
        print(link)
