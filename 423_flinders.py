...
def flinders():
	url = "https://www.flinders.edu.au/directory/index.cfm/affiliation1/list?structureid=20337&structureheader=Teaching+Section&structurename=Information+Technology+&+Software+Engineering"   # homepage url
	...
	filename = "flinders.txt"
	f = open(filename, "w")

	excel_filename = "flinders.csv"
	...
	#for_find = {'grid grid--12'}
	d = soup.find_all('div',{'class':'row table table-flex'})
	#print(d)
	# count = 0 
	# iterating for every prof
	for i in d:
		#print(d)
		find ={'col-xs-12 col-sm-12 col-md-1 col-lg-1 odd center lastrow','col-xs-12 col-sm-12 col-md-1 col-lg-1 even center lastrow'}
		a = i.get('a',{'class':find})  # a contains the name and the homepage of prof
		print(a)
		if a == None:
			continue
		link = a.get('href')  # extracting prof page link
		print(link)
		#name = i.get_text('div',{'class':'staff-card__title detached-link'})  # extracting prof name
		name=i.get('span',{'class':'fullname'})
		#print(name)
		try:
