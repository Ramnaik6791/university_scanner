...
def konstanz():
    url = "https://www.informatik.uni-konstanz.de/en/department/staff/professors/"   # homepage url
    ...
    filename = "konstanz.txt"
    f = open(filename, "w")

    excel_filename = "konstanz.csv"
    ...
    d = soup.find('div', {'class':'accordion persons tiles'})
    #print(d)
    dd = d.find_all('section', {'class': 'with-image'})
    #print(dd)

    #iterating for every prof
    for i in dd:
        a = i.find('a', {'class':'linkblock www external'})                     # a contains the name and the homepage of prof
        if a == None:
            continue
        #print(a)
        link = a.get('href')                # extracting prof page link
        #print(link)
        name = i.find('h2')                # extracting prof name
        name=name.get_text()
        name=name.strip()
        #print(link)
        #print(name)
